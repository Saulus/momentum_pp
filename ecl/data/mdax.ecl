...
/*
	export file_test := DATASET([{'A','Along',20120101,20131231},
	...
	mdax_import_file := DATASET([{'20120101','A','Along'},
															{'20120101','B','Blong'},
															{'20120101','C','Clong'},
															{'20130101','A','Along'},
															{'20130101','B','Blong'},
...
//Prep: Add dates/shares combinations that are not presenz
	...
	mdax_allsharedates_rec doJoin2(mdax_allsharedates_rec l, mdax_truesharedates r) := TRANSFORM
	 ...
	 SELF := l;
	END;
	SHARED mdax_imported_plus_notavail := join(mdax_falsesharedates, mdax_truesharedates,LEFT.shortcut = RIGHT.shortcut and LEFT.date = RIGHT.date,doJoin2(LEFT,RIGHT),LEFT OUTER);
...
//Create result: new layout with gregorian dates, and iteration, plus dedup 
	...
	//dedup and keep only false rows (=latest changes) and startdates>0
	SHARED res_File := DEDUP(
									sort(pre_file(isavailable=false,startdate_greg>0),startdate_greg,enddate_greg),
	...
	export file := UNGROUP(table(res_File,layout)):persist('~momentum::persist::mdax');
END;